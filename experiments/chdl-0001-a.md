...
- Carnegie Hall's dataset contains thousands of names for which no Wikidata item exists; consequently, these names would not be discoverable in this current presentation via the Wikidata Query Service. Our goal is to advance our discovery and presentation layer to the point where we can offer map (and other) visualizations natively and thereby expose this unique data.
